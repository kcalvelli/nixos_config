...
in
{
  ...
  # --- Bun/TS updater script (detect colors from DMS/Matugen GTK/Qt files) ---
  home.file."${themeProjectDir}/update-theme.ts".text = ''
    ...
    // Locate extension and the theme file VS Code actually loads (from package.json)
  ...
  systemd.user.paths."material-code-theme-watch" = {
    ...
    Path = {
      PathChanged = [ gtk4Css gtk3Css qt6ct qt5ct ];
      Unit = "material-code-theme-watch.service";
    };
  ...
  systemd.user.timers."material-code-theme-on-login" = {
    Unit.Description = "Run material-code theme build shortly after login";
    Timer = {
      OnBootSec = "10s";
      Unit = "material-code-theme-watch.service";
    };
